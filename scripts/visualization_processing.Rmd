...
NB This is a judgement call, esp. when making a directional graph
...
for (i in nodes) {
  ...
  nodes1 <- list$combinations %>%
    filter(strings == i) %>%
    rename(source_id = strings)
  
  nodes2 <- list$combinations %>%
    filter(!strings == i) %>%
    rename(target_id = strings)
  
  edges <- nodes1 %>%
    left_join(nodes2, by = "id") %>%
    count(source_id, target_id)
...
#create list from dataframe with edge list
links <- list$edges %>%
